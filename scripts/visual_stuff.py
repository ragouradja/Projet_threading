...
def user_param():
	...
	parser.add_argument("-p", help="Path to the PDB file (template).", type=str, required=True, metavar="pdb")
	parser.add_argument("-f", help="Path to the Fasta file (target).", type=str, required=True, metavar="fasta")

	parser.add_argument("--cpu", help=textwrap.dedent('''\
		Number of CPUs to use (Default -1 : All CPUs but one are used).
		If the number entered exceed your PC capacity, max CPU will be use.'''),type=int, metavar="int", default=-1)
	...
	parser.add_argument("--weight", help=textwrap.dedent('''\
		...
		(Option used with --blosum)'''),  type = int, default = 0, metavar="int")
	...
	parser.add_argument("--zscore", help=textwrap.dedent('''\
		...
		Execution time will be multiplicated by this number of run.'''),default=0, type=int, metavar="int")
	parser.add_argument("--dssp", help="Add gap penalty based on the secondary structure of the template.", action = "store_true")
	parser.add_argument("--log", help="Name of the log file. Default `results.log`.",default="results.log", type = str,metavar="str")
	parser.add_argument("--chain", help="Chain to select in PDB file.",default=None, type = str,metavar="str")
	...
	if "fasta" not in fasta:
		sys.exit("Please, enter a valid Fasta file.")
	if cpu != -1 and cpu <= 0:
...
def print_alignment(align_pdb, align_res,informations,options):
	...
	informations : list
		List of all informations to write in log file
	
	options : namespace
		To write options used in log file		
	"""
	seq = informations[0]
	pdb = informations[1]
